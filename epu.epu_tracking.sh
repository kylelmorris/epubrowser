...
if [ $flagcheck = 0 ] ; then
  ...
  echo "-i - input star file (not used)"
  echo "-c - star column name (not used)"
  echo "-s - suffix to remove (not used)"
  echo ""
  echo "------------------------------------------------------------------"
  exit 1
fi
...
#Create a file detailing EPU directory structure just for micrographs
find ${epu}/* -name "*.jpg" | grep Data > "${EPU_OUT}/EPU_structure_mics.dat"
# Create a file detailing micrograph names
find ${epu}/* -name "*.jpg" | awk -F"/" '{print $NF}' | grep Data | cut -f 1 -d '.' > "${EPU_OUT}/EPU_micrographs.dat"

#Get unique micrograph reference and then column for where GridSquare reference is in file path
#Get a micrograph name
p=$(sed -n 2p ${EPU_OUT}/EPU_micrographs.dat)
#Find that micrograph in the directory structure
q=$(grep ${p} ${EPU_OUT}/EPU_structure_mics.dat)
...
################################################################################
## Link all square image and xml's
################################################################################
...
################################################################################
# Link all Foil hole and Data images in square directories
################################################################################
